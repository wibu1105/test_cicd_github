CREATE PROCEDURE [Bronze].[sp_seed_initial_data]
AS
BEGIN
    -- Bắt đầu khối giao dịch để đảm bảo nếu lỗi thì không bị insert nửa vời
    BEGIN TRY
        BEGIN TRANSACTION;
    ...
    BEGIN CATCH
        ...
        -- Bạn có thể ném ra thông báo lỗi chi tiết nếu muốn
        THROW;
    END CATCH
END