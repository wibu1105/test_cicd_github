-- Bronze.usp_seed_demo_data
--
-- Seeds Bronze with demo source data (country/state/city/address/customer,
-- product catalog, sales_order/sales_order_line).
--
-- No TRUNCATE here by design: the calling notebook already clears out
-- Bronze/Gold before reloading, so this procedure only inserts.
-- Re-running it without that prior clear will hit PK/UNIQUE violations,
-- since none of these are MERGE/upsert statements.

CREATE   PROCEDURE Bronze.usp_seed_demo_data
AS
BEGIN
    SET NOCOUNT ON;
